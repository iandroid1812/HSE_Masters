...
class Exp_financial(Exp_Basic):
    def __init__(self, args):
        ...
        self.writer = SummaryWriter(f'.exp/run_financial/{args.model_name}')
    ...
    def validate(self, data, X, Y, evaluate=False):
        ...
        mse = MSE_metric(forecast_Norm.cpu().numpy(), target_Norm.cpu().numpy())
        mae = MAE_metric(forecast_Norm.cpu().numpy(), target_Norm.cpu().numpy())
